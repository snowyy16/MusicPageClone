...
<<<<<<< HEAD
        nameSong: 'Creep',
        nameArtist: 'Radiohead',
        nameFile:'radiohead/creep.mp4',
        time:'4:22',
        img: 'radiohead/creep.jpg'
    },
    {
        nameSong: 'Nàng thơ',
        nameArtist:'Amee',
...
song.ontimeupdat=function (){seekBar.value = song.curtime}
handleSeekbar=function(){song.curtime=seekBar.value} 

function displayTimer(){
    seekBar.min=0;
    seekBar.max=song.duration;
    curtime.textContent = formatTime(song.curtime);
    durtime.textContent= formatTime(song.duration);
}
function formatTime(time){
    var min = Math.floor(time/60);
    var sec = Math.floor(time%60);
    if(sec<10) return sec = `${min}:0${sec}`;
    else return `${min}:${sec}`;
}
function changeBar(){
    songBar.curtime=songBar.value;
}var itemTopSong = [];
function renderTopSong(){
    topListSong.innerHTML = '';
...
function mulVol(){
    if(isMute){
        volIcon.innerHTML = `<i class="fa-solid fa-volume-mute"></i>`;
        isMute=false;
        changeVol(volRange.value);
    }else{
...
function pushArray(){
    ...
    while(arrayAdd.length<lengthOfarrayAdd){
        var termpValue=Math.floor(Math.random()*listRandom.length)
...
function playAnotherSong(index,i){
    ...
    topSongThumb.setAttribute('sec',`./img/$img{listSongOf[index].img}`)
